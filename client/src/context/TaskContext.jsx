...
export const TaskProvider = ({children}) => {
  ...
  const moveTask = async (taskId, newState) => {
    try {
      const response = await moveTaskApi(taskId, newState);
  ...
  const addComment = async (taskId, comment) => {
    try {
      const response = await addCommentApi(taskId, comment);

      if (response.success) {
        // Actualizar la tarea con los nuevos comentarios
        setTasks((prev) =>
          prev.map((task) =>
            task._id === taskId ? {...task, comments: response.comments} : task
          )
        );
  ...
  const updateSubtask = async (taskId, subtaskId, completed) => {
    try {
      const response = await updateSubtaskApi(taskId, subtaskId, completed);

      if (response.success) {
        setTasks((prev) =>
          prev.map((task) =>
            task._id === taskId ? {...task, subtasks: response.subtasks} : task
          )
        );
        return {success: true};
      }
   ...
   */
  const clearFilters = () => {
    setFilters({status: null, priority: null, assignedTo: null});
    loadTasks({});
  };
